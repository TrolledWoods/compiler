        ...
        for (line_counter, line) in file_content.lines().enumerate() {
            if let Some(log) = &next_log {
                ...
                if wanted_next_line == line_counter {
                    print_message(n_digits, log.message, log.position, log.start_char, log.end_char);
...
fn print_message(n_digits: usize, message: &str, pos: LogPosition, start: Option<usize>, end: Option<usize>) {
